...
const CreateProductServiceModel = lazy(() =>
  import("../models/CreateProductServiceModel")
);
const fetchSize = 10;
...
const ProductServiceTable = () => {
  ...
  const [isCreateProductServiceModelOpen, setIsCreateProductServiceModelOpen] =
    useState(false);
  ...
  return (
    <>
        ...
        renderTopToolbarCustomActions={({ table }) => (
          <Box display="flex" alignItems={"stretch"} gap="5px">
            {loggedUser && loggedUser?.role !== "member" && (
              <button
                onClick={() => setIsCreateProductServiceModelOpen(true)}
      ...
      />
      <Suspense>
        {loggedUser?.role !== "member" && isCreateProductServiceModelOpen && (
          <Model
            isOpen={isCreateProductServiceModelOpen}
            setIsOpen={setIsCreateProductServiceModelOpen}
            title="Create Product Service"
          >
            <CreateProductServiceModel
              setIsOpen={setIsCreateProductServiceModelOpen}
            />
          </Model>
        )}
